...
async function initEventDetailPage() {
    ...
    if (user && ev.participants && ev.participants.some(p => p.userId === user.id)) {
...
function populateHero(ev) {
    ...
    const safeSportEmoji = typeof sportEmoji === 'function' ? sportEmoji(ev.sport) : '🏅';
    ...
    if (metaItems.length >= 5) {
        ...
        metaItems[1].textContent = ev.durationMinutes ? `${ev.durationMinutes} min` : 'TBD';
...
function populateActionCard(ev) {
    ...
    if (el('action-date')) el('action-date').textContent = formatEventDate(ev.dateTime || ev.date);
    if (el('action-duration')) el('action-duration').textContent = ev.durationMinutes ? `${ev.durationMinutes} min` : 'TBD';
    if (el('action-location')) el('action-location').textContent = ev.location;
    if (el('action-level')) el('action-level').textContent = ev.skillLevel || 'All levels';
    if (el('action-join-type')) el('action-join-type').textContent = ev.requiresApproval ? 'Request' : 'Instant';
    ...
    if (hostAvatar) hostAvatar.textContent = (ev.organizerName || '?')[0].toUpperCase();
        ...
        ev.participants.forEach((p, index) => {
            ...
            html += `
                <div class="attendee-card ${hostClass}">
                    <div class="attendee-avatar avatar-${color}">${(p.username || '?')[0].toUpperCase()}</div>
                    <div>
...
function initEventMap(ev) {
    ...
    const icon = L.divIcon({
        ...
        html: `<div class="map-pin-bubble active">${safeSportEmoji}</div>`,
        iconSize: [30, 30],
        iconAnchor: [15, 30]
    });
...
async function populateComments(eventId) {
    ...
    if (comments && comments.length > 0) {
        ...
        commentsList.innerHTML = '<div style="text-align:center;padding:20px;opacity:.4;">No comments yet. Be the first!</div>';
    }
}
...
async function populateSimilarEvents(sport, currentId) {
    ...
    const safeSportEmoji = typeof sportEmoji === 'function' ? sportEmoji(sport) : '🏅';
    
    if (otherEvents.length > 0) {
        similarContainer.innerHTML = otherEvents.map(s => `
        ...
        similarContainer.innerHTML = '<div style="text-align:center;padding:16px;opacity:.4;">No similar events found.</div>';
    }
}
...
window.postComment = async function() {
    ...
    if (typeof isLoggedIn === 'function' && !isLoggedIn()) { window.location.href = 'auth.html'; return; }
...
window.saveEvent = async function() {
    ...
    if (!window._eventId) return;
    const result = await apiPost(`/event/${window._eventId}/save`);
    if (result && result.ok) alert('Event saved!');
    else alert('Already saved or error.');
};

window.reportEvent = async function() {
    if (typeof isLoggedIn === 'function' && !isLoggedIn()) return;
    if (!window._eventId) return;
    const reason = prompt('Why are you reporting this event?');
    if (!reason) return;
    const result = await apiPost(`/event/${window._eventId}/report`, { reason });
